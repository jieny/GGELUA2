...
function SDL精灵:SDL精灵(p, x, y, w, h, _win)
...
function SDL精灵:置坐标(x, y)
    ...
    if x and y then
        ...
        self._x, self._y = x, y
        self._dr:SetRectXY(math.floor(x), math.floor(y))
    end
end

function SDL精灵:显示(x, y, zx, zy, zv)
    if zx and zy and zv then
        -- self:置缩放(zx, zy, zv)
    end
    self:置坐标(x, y)
    ...
    if tex then
        ...
        if self._f or self._deg then --src,dst,旋转，翻转，翻转中心
            ...
            if self._hl then --高亮
                ...
                tex:SetTextureColorMod(55, 55, 55, 256)
                win:显示纹理(tex, self._sr, self._dr, self._deg, self._f, self._ax, self._ay)
            end
        else
...
function SDL精灵:置缩放(x, y, v)
    y = math.floor(self.高度 * (y or x) - self.高度 * (y or x)/2)
    x = math.floor(self.宽度 * x - self.宽度 * x/2)
